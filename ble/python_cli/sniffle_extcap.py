...
class SniffleExtcapPlugin():
    ...
    def parseArgs(self, args=None):
        ...
        if self.args.preload:
            ...
            if len(preload) > SniffleHW.max_interval_preload_pairs:
                raise UsageError(
                    'Please specify no more than %d interval preload pairs' % (SniffleHW.max_interval_preload_pairs))
    ...
    def extcap_interfaces(self):
        ...
        lines.append(
            "control {number=%d}{type=button}{role=logger}{display=Log}{tooltip=Show capture log}" % CTRL_NUM_LOGGER)
        return '\n'.join(lines)
